...
cdef class MINE:
    ...
    def compute_score(self, x, y):
        ...
        """
        cdef int i, j
        cdef np.ndarray[np.float64_t, ndim=1] xa, ya
...
def pstats(X, alpha=0.6, c=15, est="mic_approx"):
    ...
    cdef np.ndarray[np.float64_t, ndim=2] Xa
    cdef np.ndarray[np.float64_t, ndim=1] mica, tica
...
def cstats(X, Y, alpha=0.6, c=15, est="mic_approx"):
    ...
    cdef np.ndarray[np.float64_t, ndim=2] Xa, Ya
    cdef np.ndarray[np.float64_t, ndim=2] mica, tica
